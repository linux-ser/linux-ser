module.exports = async function moonCommand(sock, chatId, message) {
    try {
        ...
        await delay(1000);
        await sock.sendMessage(chatId, {
            text: "🌝 𝐌ᴏᴏɴ 𝐒ᴇɴᴛ ʙʏ 𝐋ɪɴᴜх 𝐒ᴇʀ 🧃🕊️",
            edit: sentMsg.key,
            mentions: [] // ആരെയും മെൻഷൻ ചെയ്യില്ല
        });

    } catch (error) {
        console.error("Moon command handling error:", error);
        await sock.sendMessage(chatId, { text: "❌ Error while running moon command" }, { quoted: message });
    }
};
